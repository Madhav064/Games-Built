...
import { recordWin } from "../core/progress";
...
function loadArmyPrefs(): Record<Faction, ArmySkinId> {
  ...
  try {
    const raw = getStorage().getItem(ARMY_PREFS_KEY);
...
function saveArmyPrefs(skins: Record<Faction, ArmySkinId>): void {
  try {
    getStorage().setItem(ARMY_PREFS_KEY, JSON.stringify(skins));
  } catch {
...
function loadSeatSwing(): boolean {
  ...
  try {
    return getStorage().getItem(TABLE_PREFS_KEY) === "swing";
  } catch {
    return false;
  }
}
...
function saveSeatSwing(enabled: boolean): void {
  try {
    getStorage().setItem(TABLE_PREFS_KEY, enabled ? "swing" : "hold");
  } catch {
    // Private browsing — the choice just will not survive the reload.
  }
}
...
function loadPremoves(): boolean {
  ...
  try {
    return getStorage().getItem(PREMOVE_PREFS_KEY) !== "off";
  } catch {
    return true;
  }
}
...
function savePremoves(enabled: boolean): void {
  try {
    getStorage().setItem(PREMOVE_PREFS_KEY, enabled ? "queue" : "off");
  } catch {
    // Private browsing — the choice just will not survive the reload.
  }
}
...
function loadPremoveDepth(): number {
  ...
  try {
    const value = Number(getStorage().getItem(PREMOVE_DEPTH_KEY));
...
function savePremoveDepth(depth: number): void {
  try {
    getStorage().setItem(PREMOVE_DEPTH_KEY, String(depth));
  } catch {
    // Private browsing — the choice just will not survive the reload.
  }
}
...
function loadThinkFloor(): number {
  ...
  try {
    const raw = getStorage().getItem(THINK_PREFS_KEY);
...
function saveThinkFloor(ms: number): void {
  try {
    getStorage().setItem(THINK_PREFS_KEY, String(ms));
  } catch {
    // Private browsing — the choice just will not survive the reload.
  }
}
...
function loadRenderPrefs(): RenderPrefs {
  ...
  try {
    ...
    const raw = getStorage().getItem(RENDER_PREFS_KEY);
...
function saveRenderPrefs(prefs: RenderPrefs): void {
  try {
    getStorage().setItem(RENDER_PREFS_KEY, JSON.stringify(prefs));
  } catch {
...
export function GameShell() {
  ...
  const lastRecordedResult = useRef<GameResult | null>(null);

  useEffect(() => {
    if (snapshot.status === "over" && snapshot.result && snapshot.result !== lastRecordedResult.current) {
      lastRecordedResult.current = snapshot.result;
      if (snapshot.result.winner === snapshot.playerColor && snapshot.mode !== "demo") {
        recordWin(snapshot.mode === "ai");
      }
    }
  }, [snapshot.status, snapshot.result, snapshot.playerColor, snapshot.mode]);

  useEffect(() => {
    if (phase === "playing" && !getStorage().getItem("kg.tutorialSeen")) {
      setShowTutorial(true);
      getStorage().setItem("kg.tutorialSeen", "true");
    }
  }, [phase]);
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && (window as any).CrazyGames?.SDK) {
      try {
        ...
        console.warn("CrazyGames SDK gameplay tracking error", e);
      }
    }
  }, [phase]);
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0">
        ...
        {showManual ? (
          <GameManualModal onClose={() => setShowManual(false)} />
        ) : null}
...
function LoadingScreen({ progress }: { progress: number }) {
  return (
    <div className="mc-fade absolute inset-0 flex flex-col items-center justify-center gap-5 bg-[#05060a]/85 px-6">
      <p className="mc-display text-[0.62rem] tracking-[0.5em] text-[#a89268]">MUSTERING THE PIECES</p>
      <h1 className="mc-display mc-title-glow text-4xl text-[#f4e3bd]">BATTLE CHESS 3D</h1>
