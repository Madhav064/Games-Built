    ...
    <div className="mc-slate mc-goldleaf px-4 py-3">
      <div className="flex items-center justify-between">
        ...
        <span className="mc-display text-[0.72rem] text-[#e2c98f]">
          {diff === 0 ? "even" : diff > 0 ? `white +${diff}` : `black +${-diff}`}
        </span>
      </div>
...
function FieldTally({ snapshot, getElapsed }: { snapshot: GameSnapshot; getElapsed: () => ElapsedState }) {
  ...
  return (
    <div
      ...
      aria-label={`Field tally. White: ${losses.w} lost, ${formatElapsed(elapsed.whiteMs)} on the field. Black: ${
        losses.b
...
function TallyRow({
  ...
  return (
    <div className="mc-tally-row" data-faction={faction} data-live={onMove || undefined}>
      ...
      <span className="mc-tally-name">{faction === "w" ? "White" : "Black"}</span>
...
function ClockFace({
  ...
  return (
    <div className="flex items-center gap-1.5" style={{ opacity: active ? 1 : 0.55 }}>
      ...
      <div>
        <p className="mc-display text-[0.5rem] tracking-[0.2em] text-[#a89268]">{faction === "w" ? "WHITE" : "BLACK"}</p>
